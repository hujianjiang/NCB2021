...
filepath = 'Y:\Optogenetics\HT1080PARac1\analysis\windowing\';
InitialArray = 1:29;
EXPname = 'HT1080PARac1in1%FBSon2.3kPAA';
...
for i = [3, 5 : 15, 17, 19 : 21, 23: 55, 57:85, 87: 91] %[3 , 5 : 13 , 15 , 17 : 21 , 23 , 25 : 26, 28 : 33, 35 : 50 , 52 : 91] %[1 , 3 , 5 : 91]
    ...
    for j3 = WindowsROI{i}
        %         [pp,p] = csaps(1:n2-1 , protIntegrated{i}(j3, :),0.01);
        %         [pp,p] = csaps(1:61 , protIntegrated{i}(j3, 1 : 61),0.01);
        ...
        if size(pp.coefs,2) == 4
            ...
            if length(peakposition) > 1
                for i3 = 1 : length(peakposition)-1
                    ...
                    if peakposition(i3+1) <=61 %&& peaklocfilted{i}{j3}(peakposition(i3+1)) - valleyloc{i}{j3}(tempvalleyloc(1)) >= distance
                        %                         if peakvalfilted{i}{j3}(peakposition(i3+1)) - valleyval{i}{j3}(tempvalleyloc(1)) >= valuerange
...
% retrN2 = find(nanmean(InteDretractionMaxV(:,n-8:n+8))==min(nanmean(InteDretractionMaxV(:,n-8:n+8))));
...
PlotArray_title = {'PA-Rac1(Q61L) retraction distance','PA-Rac1(Q61L) retraction velocity','PA-Rac1(Q61L) retraction acceleration','PA-Rac1(Q61L) retraction force','PA-Rac1(Q61L) protrusion distance','PA-Rac1(Q61L) protrusion velocity','PA-Rac1(Q61L) protrusion acceleration','PA-Rac1(Q61L) protrusion force'};
PlotArray_legendLoc = {'northwest','best','northwest','northwest','northwest','northwest','southwest','southwest'};
...
PlotArray_legendPos = {[0.32 0.68 0.1 0.1],[0.42 0.38 0.1 0.1],[0.32 0.68 0.1 0.1],[0.32 0.68 0.1 0.1],[0.32 0.6 0.1 0.1],[0.58 0.41 0.1 0.1],[0.32 0.68 0.1 0.1],[0.32 0.68 0.1 0.1]};
...
for i = 1 : size(PA,2)
    ...
    legend({'control',label{1:size(PlotArray_SelectedT{i},2)}},'Position',PlotArray_legendPos{i},'FontSize',40);
    legend boxoff
